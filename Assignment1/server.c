...
{
	int sockfd,n;
	struct sockaddr_in serverAddr;  //specify local server address to connect a socket.
	struct sockaddr_storage serverStorage; //stores socket address information
	...
	for(; ;) {
		...
		int length = strlen(data.payload);   // computing length of data packet recieved

		// Duplicate packet reject code
		...
		// endpacketid missing reject code
